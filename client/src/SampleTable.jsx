...
export default function Component() {
  ...
  const filterUsers = (search, ratings, year, branch) => {
    let result = users.filter((user) => {
      ...
      const matchesRating = user.rating >= ratings[0] && user.rating <= ratings[1];
      ...
      <div className="grid gap-4 mb-4 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <div className="relative">
            ...
            <Input id="search" placeholder="Search users..." value={searchTerm} onChange={handleSearch} className="pl-8" />
          </div>
        </div>
        <div>
          <Label>Rating Range</Label>
          <Slider min={0} max={3500} step={100} value={ratingRange} onValueChange={handleRatingChange} className="mt-2" />
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredUsers.map((user) => (
          <Card key={user.id}>
            ...
            <CardFooter>
              <a href={`https://codeforces.com/profile/${user.codeforcesId}`} target="_blank" rel="noopener noreferrer" className="flex items-center text-blue-500 hover:underline">
